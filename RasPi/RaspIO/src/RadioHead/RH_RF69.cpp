...
uint8_t RH_RF69::checkForReception()
{
   uint8_t value = digitalRead(_interruptPin);
   if(value==1) handleInterrupt();
   return value;
}
...
bool RH_RF69::setModemConfig(ModemConfigChoice index)
{
   ...
   ModemConfig cfg;
   memcpy(&cfg, &MODEM_CONFIG_TABLE[index], sizeof(RH_RF69::ModemConfig));
