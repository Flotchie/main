...
{
   if(iType>OLED_LAST_OLED)iType=OLED_LAST_OLED;
   return oled_type_str[iType];
}
...
ArduiPi_OLED::ArduiPi_OLED(RHGenericSPI& spi)
: _spi(spi), _i2c_addr(0), vcc_type(0), oled_type(0), grayH(0), grayL(0), verbose(false)
{
...
bool ArduiPi_OLED::select_oled(uint8_t OLED_TYPE)
{
  ...
  vcc_type = SSD_Internal_Vcc;
   ...
   if (!poledbuff)
      return false;
   if(verbose) ::printf("oled_buff_size[%d]\n", oled_buff_size);
   return true;
...
{
  ::printf("ArduiPi_OLED::init(dc=%d, rst=%d, cs=%d, oled=%d)\n",
  DC, RST,  CS,  OLED_TYPE);
  rst = RST;  // Reset Pin
...
void ArduiPi_OLED::begin()
{
   ...
   {
      sendCommand(SSD_Display_Off);
      sendCommand(SSD1306_Set_Memory_Mode, 0x00);       
      sendCommand(SSD1306_Set_Display_Clock_Div, 0x80);    /*set osc division*/ 
      sendCommand(SSD_Set_Muliplex_Ratio, 0x3F);/*multiplex ratio duty = 1/64*/    
      sendCommand(SSD1306_Set_Display_Offset,0x00);    /*set display offset 0x20 */
      sendCommand(SSD1306_Set_Start_Line | 0x0);       /*set display start line*/
      sendCommand(SH1106_Set_Charge_Pump, 0x14);    /*set charge pump external VCC*/
      ...
      sendCommand(SSD1306_Set_Com_Pins,0x12);    /*set COM pins*/
      sendCommand(SSD_Set_ContrastLevel, contrast); /*contrast control*/
      sendCommand(SSD1306_Set_Precharge_Period, 0xF1);    /*set pre-charge period*/
      sendCommand(SSD1306_Set_Vcomh_Deselect_Level,0x40);    /*set vcomh*/
      //sendCommand(0x32);    /*0X30---0X33 set VPP 8V */
      sendCommand(0xA4);     /*normal / reverse*/
   ...
   else if( (oled_type == OLED_SH1106_SPI_128x64) )
   {
        sendCommand(0xAE);
        sendCommand(0x20);
        sendCommand(0x10);
...
{ 
   //if(verbose) ::printf("sendCommand(%X)\n", c);
...
{ 
   //if(verbose) ::printf("sendCommand(%X,%X)\n", c0, c1);
   char buff[3] ;
   buff[1] = c0;
   buff[2] = c1;
...
void ArduiPi_OLED::display(void) 
{
   ...
   if (oled_type == OLED_SH1106_SPI_128x64)
   {
      ...
      {
         sendCommand(SH1106_Set_Page_Address+page);//set page addressSSD_Data_Mode;
         sendCommand(SSD1306_Set_Lower_Column_Start_Address  | 0x2) ;//set lower column address
         sendCommand(SSD1306_Set_Higher_Column_Start_Address | 0x0) ;//set higher column address
         // Setup D/C line to high to switch to data mode
         bcm2835_gpio_write(dc, HIGH);
         _spi.chipSelect(cs);
...
void ArduiPi_OLED::clearRow(uint8_t row) 
{
  uint8_t page = SH1106_Set_Page_Address + row;
  sendCommand(page);
  ...
  for( uint8_t i=0;i<4;++i) sendData(0x00);
}
